...
def block_interface(doc):
    ...
    image(doc, IMG_UI, "Скриншот 1. Матрица по client_id=221, dog_id=88: закрепленная иерархия, итоговые суммы, `Переплата`/`Долг` без документных статусов 1С.", 6.45)
    ...
    add_table(
        ...
        [
            ...
            ("Сверка ERP ↔ 1С", "Запускается на отдельной странице монитора. Матрица сальдо не показывает документные статусы 1С."),
    ...
    add_table(
        ...
        [
            ...
            ("`Номер/Дата/Договор расходится`", "Документ найден, но расходится конкретное поле.", "backend обязан вернуть конкретный вид расхождения; общий текст `Реквизиты расходятся` не использовать"),
...
def block_export(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Запуск", "Экспорт принимает тот же фильтр клиента/ЮЛ/договора/периода, что и экран сальдо. Документная сверка 1С в эту выгрузку не включается."),
