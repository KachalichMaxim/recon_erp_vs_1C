...
def title_page(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Не входит", "PostgreSQL, Google Drive/Sheets, отдельный Python-модуль, замена действующего ERP-интерфейса."),
    ...
    add_callout(
        doc,
        "Главное правило",
        "Матрица не должна подтягивать внешний Google Sheet как источник данных. Google-таблица используется только как исторический референс формы и логики. Производственный расчет должен воспроизводиться из MariaDB ERP и подтверждаться существующим сервисом 1С.",
    )
...
""",
    ...
    add_table(
        ...
        [
            ("Возмещаемые", "f_isvozm = 1", "`veda_akts_details_opers.f_sum` и/или прямой акт по операции", "попадают в колонку `Возмещаемые расходы`"),
            ("Невозмещаемые клиентские", "f_isvozm = 2 + прямой клиентский акт/реализация", "`settlements.control_non_reimbursable_total` / `get_realizsum/get_profit`", "попадают в колонку `Невозмещаемые расходы`"),
...
def add_column_rules(doc):
    ...
    add_table(
        ...
        [
            ...
            ("(+/-)", "оплаты - возмещаемые - невозмещаемые", "для родительских строк сумма дочерних поставок."),
...
def add_excel(doc):
    ...
    add_body(
        doc,
        "Копирование в буфер должно формировать плоский TSV: значения объединенных ячеек повторяются в каждой строке, чтобы бухгалтер мог вставить данные в Excel/Google Sheets без потери контекста.",
    )
...
def add_control_case(doc):
    ...
    add_body(
        doc,
        "Проверка выполнялась на свежих поставках договора 660/1, а не на историческом 2020 годе. Google Sheet использовался как референс формы; итоговый расчет ниже воспроизводится из MariaDB ERP.",
    )
    add_table(
        doc,
        ["Параметр", "Значение"],
        [
            ...
            ("Итоги", "оплата 2 268 353,17; возмещаемые 2 152 392,27; невозмещаемые 196 372,85; дельта -80 411,95"),
...
def add_acceptance(doc):
    ...
    checks = [
        ...
        "Свежий кейс 660/1 воспроизводит итоги: оплата 2 268 353,17; возм. 2 152 392,27; невозм. 196 372,85; дельта -80 411,95.",
