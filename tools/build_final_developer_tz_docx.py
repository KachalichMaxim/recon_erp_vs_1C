...
VERSION_LABEL = "LIVE MariaDB + тестовый монитор 1С / 2026-06-25 / финальное ТЗ для ERP-разработчика"
...
def title_page(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Контур", "Встроенный экран текущей PHP ERP, MariaDB, существующий SOAP-обмен с 1С; тестовый стенд может использовать загруженную выгрузку 1С в PostgreSQL только как кэш источника 1С."),
...
def add_product_split(doc):
    ...
    add_table(
        ...
        [
            ...
            (
                ...
                "MariaDB ERP + существующий SOAP-сервис 1С. На тестовом стенде допускается PostgreSQL-кэш загруженной выгрузки 1С; он не является новым production-источником.",
    ...
    add_callout(
        doc,
        "Терминология",
        "`Сальдо по поставкам` - ERP-расчет по выбранным поставкам. `Монитор ERP ↔ 1С` - отдельный процесс сравнения двух систем. Бейдж вида `9/9 ОК` в матрице документов не означает, что сальдо ERP равно сальдо 1С.",
        fill=WARN_FILL,
    )
...
def add_functional_requirements(doc):
    ...
    add_table(
        ...
        [
            ...
            (
                "FR-06",
                "Страница `Монитор ERP ↔ 1С` запускается отдельно от матрицы сальдо и работает через существующий PHP/SOAP-слой или согласованный кэш выгрузки 1С.",
...
def add_nonfunctional_requirements(doc):
    ...
    add_table(
        ...
        [
            ...
            (
                "Совместимость",
                "Production-реализация остается в текущем PHP ERP + MariaDB + SOAP 1С. Python-скрипты, Google Drive/Sheets и тестовые JSON/XLSX не являются частью промышленного контура. PostgreSQL можно использовать только как технический кэш загруженной выгрузки 1С, если это отдельно утверждено архитектурой.",
            ),
            (
                "Наблюдаемость",
...
def add_ui(doc):
    ...
    add_image(doc, IMG_SALDO, "Рисунок 1. Тестовый вид экрана `Сальдо по поставкам`: несколько поставок договора 660/1, ERP-расчет без обращения к 1С.")
    add_image(doc, IMG_MONITOR, "Рисунок 2. Тестовый вид страницы `Монитор ERP ↔ 1С`: проверка документов по нескольким поставкам с источником 1С.")
...
def add_sources_and_selects(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Размер страницы", "Live-экран показывает небольшой срез поставок: рекомендуемый стартовый лимит 3-5, все остальное через пагинацию или фоновую XLSX/runner-задачу. Проверка `limit=10` на договоре 660/1 не уложилась в приемлемое время."),
...
def add_control_case(doc):
    ...
    add_heading(doc, "14.1 Проверка тестового монитора ERP ↔ 1С", 2)
    add_body(
        doc,
        "Для страницы монитора выполнена тестовая проверка по тому же договору 660/1 на небольшом live-срезе. Источник 1С в тесте - загруженная выгрузка 1С в PostgreSQL; в production этот источник должен быть заменен или подтвержден как согласованный кэш SOAP-выгрузки.",
    )
    add_table(
        doc,
        ["Параметр", "Результат"],
        [
            ("URL теста", "`client_id=221&dog_id=88&limit=3&scope=legal&compare_1c=1`"),
            ...
            ("Вывод", "Такой результат нельзя показывать как `1С ОК`; нужен отдельный статус `Договор/заявка расходится` и запись в журнал расхождений."),
            ("Производительность", "Live `limit=3` занимает десятки секунд, `limit=10` не подходит для синхронного экрана; полный договор/историю считать runner-ом."),
        ],
        [2300, 7060],
        font_size=7.65,
    )
...
def add_acceptance(doc):
    ...
    checks = [
        ...
        "На live-экране действует малый лимит и пагинация: по контрольному URL `client_id=221&dog_id=88&scope=legal` срез 3 поставок отрабатывает, а попытка 10 поставок уже неприемлема для синхронного запроса.",
        ...
        "Для настоящей сверки ERP ↔ 1С есть отдельная страница `Монитор ERP ↔ 1С` с запуском `run_id`, статусом источников, блоком `ERP без 1С`, блоком `1С без ERP`, блоком полевых расхождений и блоком сальдо.",
