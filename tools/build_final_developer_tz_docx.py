...
def add_mapping(doc):
    ...
    add_table(
        ...
        [
            ...
            ("view_specs / veda_specs", "договор-заявка / спецификация", "spec_id, spec_number, spec_num_short, spec_type/spec_subtype или f_typez/f_subtype, f_kod1cb, f_kod1cp", "№ спецификации / заявки"),
    ...
    add_heading(doc, "4.2 Что передаем и что сравниваем", 2)
    add_table(
        doc,
...
def add_sources_and_selects(doc):
    ...
    add_heading(doc, "5.0 Ограничения выборки", 2)
    add_table(
        doc,
...
""",
    ...
    add_table(
        ...
        [
            (
                "1. Контекст",
                "`veda_contacts -> veda_clients -> veda_dogs -> view_specs/veda_specs`; фильтры `contact_id`, `client_id`, `dog_id`, период или список `spec_id/f_num`.",
    ...
    add_table(
        ...
        [
            ("A `№ спецификации`", "`{view_specs.spec_type} №{spec_number}`; если тип пустой, `Поставка №{veda_specs.f_num}`.", "Объединить по числу строк счетов поставки."),
...
def add_column_rules(doc):
    ...
    add_table(
        ...
        [
            ...
            ("№ спецификации", "view_specs.spec_type/spec_subtype/spec_number/spec_num_short, veda_specs.f_id, f_kod1cb/f_kod1cp", "тип строки брать из ERP: `Заявка`, `Спецификация`, `Сертификация` и т.д.; не хардкодить `Спецификация {num}`."),
...
def add_excel(doc):
    ...
    add_table(
        ...
        [
            ("№ спецификации", "объединить по строкам счетов; текст вида `{тип ERP} №{номер}`, например `Заявка №1063`; тип брать из `view_specs.spec_type/spec_subtype`"),
...
def add_acceptance(doc):
    ...
    checks = [
        ...
        "В названии строки поставки используется тип из ERP (`view_specs.spec_type/spec_subtype`), например `Заявка №1063`, а не хардкод `Спецификация {num}`.",
