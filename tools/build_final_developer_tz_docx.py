...
def add_user_requirements(doc):
    ...
    add_body(
        doc,
        "Цель доработки - дать бухгалтеру рабочий экран сверки взаиморасчетов по клиенту в разрезе поставок: увидеть счета, оплаты, возмещаемые и невозмещаемые расходы, закрывающие документы, долг или переплату, а также быстро понять, какие документы не совпали с 1С.",
    )
    add_table(
        doc,
        ...
        [
            (
                ...
                "Видит один экран с итогами и может раскрыть проблемную поставку до документов.",
            ),
            (
                "Бухгалтер",
                "Понять, почему поставка не сходится: нет документа, другая сумма, другой НДС, нет ключа 1С, долг или переплата.",
                "Получает конкретный статус и поле расхождения, а не общий текст без причины.",
            ),
            (
                "Бухгалтер",
            ...
            (
                ...
                "Получает журнал расхождений, по которому можно анализировать причины и объем проблем.",
            ),
            (
                "Разработчик ERP",
                "Реализовать экран без догадок по источникам данных.",
                "Использует описанные таблицы, поля, SQL-связи, SOAP-методы, формулы и критерии приемки.",
            ),
        ],
        [1700, 4450, 3210],
        font_size=7.9,
    ...
    add_table(
        ...
        [
            (
                "Экран внутри текущей PHP ERP, работающий с MariaDB ERP и существующим SOAP-сервисом 1С.",
                "Отдельный внешний модуль сверки, новая база данных, замена действующего ERP-интерфейса.",
            ),
            (
                "Расчет матрицы и XLSX по выбранным клиентам, ЮЛ, договорам, поставкам и периоду.",
                "Использование Google Sheets, аналитических XLSX или Python-скриптов как production-источника данных.",
            ),
            (
                "Сверка ERP-документов с 1С: счета покупателю, оплаты, акты/УПД/СФ, договоры и итоговые сальдо.",
...
def add_functional_requirements(doc):
    ...
    add_table(
        ...
        [
            (
                "FR-01",
                "Страница открывается из раздела сверок, карточки клиента, договора или поставки. В URL допускаются параметры `contact_id`, `client_id`, `dog_id`, `spec_id`, `period`, `limit`, `scope`, `compare_1c`.",
            ...
            (
                "FR-06",
                "Система сверяет с 1С через существующий PHP/SOAP-слой, а не через прямой доступ к базе 1С.",
                "Для документов возвращаются статусы `ОК`, `Нет в 1С`, `Нет в ERP`, `Сумма расходится`, `Вопрос по НДС`, `Нет СФ/УПД` и другие коды из раздела статусов.",
            ),
            (
                "FR-07",
                "Кнопка `Ошибки` оставляет на экране строки, у которых есть хотя бы один проблемный статус.",
                "Пользователь видит причину, ERP-значение, 1С-значение и поле расхождения.",
            ),
            (
                "FR-08",
            ...
            (
                "FR-10",
                "Каждая сверка пишет журнал запуска и журнал расхождений.",
...
def add_ui(doc):
    add_heading(doc, "6. Требования к интерфейсу", 1)
    add_table(
        doc,
        ["Элемент", "Требование"],
        [
...
def add_excel(doc):
    add_heading(doc, "12. Excel-выгрузка", 1)
    add_body(
        doc,
        "Excel не должен быть простым экспортом экранной матрицы. Требуется бухгалтерская форма: счета и оплаты идут строками, а расходы, СФ/УПД и дельта объединяются по количеству строк счетов внутри поставки.",
    )
    add_table(
        doc,
...
def add_acceptance(doc):
    ...
    checks = [
        ...
        "Бейдж `1С: X/Y совпало` означает, что X строк сверки совпали из Y найденных строк ERP ↔ 1С; `Долг` и `Переплата` считаются как оплата минус возмещаемые и невозмещаемые расходы.",
