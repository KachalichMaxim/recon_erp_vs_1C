...
VERSION_LABEL = "LIVE MariaDB / 2026-06-24 / финальное ТЗ для ERP-разработчика"
OUT_VERSIONED = ROOT / "docs" / "TZ_ERP_1C_Akt_Sverki_Matrix_LIVE_MARIADB_2026-06-24.docx"

IMG_UI = ROOT / "screenshots" / "ui" / "matrix_aero_trade_660_1_live_mariadb.png"
...
def title_page(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Назначение", "Разделить две задачи: бухгалтерскую сальдовку по поставкам и техническую сверку полноты ERP ↔ 1С."),
            ("Итог", "Экран сальдо по поставкам с XLSX и отдельная страница сверки ERP ↔ 1С с протоколом расхождений."),
    ...
    add_callout(
        doc,
        "Главное правило",
        "Текущая матрица не должна называться полноценной сверкой ERP ↔ 1С. Это ERP-сальдо по поставкам. Проверка соответствия двух учетных систем должна быть отдельным экраном и отдельным процессом сверки.",
    )
...
def add_user_requirements(doc):
    ...
    add_body(
        doc,
        "Цель доработки делится на две пользовательские задачи. Первая - дать бухгалтеру рабочий экран сальдо по поставкам: счета, оплаты, возмещаемые и невозмещаемые расходы, долг или переплата. Вторая - дать отдельный инструмент проверки, что документы и сальдо ERP действительно совпадают с 1С.",
    )
    add_table(
        doc,
        ...
        [
            ...
            (
                "Бухгалтер",
                "Запустить отдельную сверку ERP ↔ 1С по ЮЛ, договору, периоду или поставке.",
                "Получает протокол: что есть в ERP, чего нет в 1С, что есть в 1С, чего нет в ERP, где расходятся суммы, даты, номера, НДС и сальдо.",
            ),
            (
                "Бухгалтер",
    ...
    add_table(
        ...
        [
            ...
            (
                ...
                "Использование Google Sheets, аналитических XLSX или Python-скриптов как production-источника данных.",
            ),
            (
...
def add_product_split(doc):
    ...
    add_table(
        ...
        [
            ...
            (
                "Сверка ERP ↔ 1С",
                "Проверить, что учетные системы совпадают: все документы ERP есть в 1С, все документы 1С есть в ERP, суммы/даты/НДС/договоры совпадают, сальдо на начало и конец совпадает.",
                "MariaDB ERP + существующий SOAP-сервис 1С. При необходимости отдельный 1С-метод для оборотов/сальдо.",
            ...
            (
                ...
                "Те же источники, но в batch-режиме с сохранением снапшота результата.",
    ...
    add_callout(
        doc,
        "Терминология",
        "`Сальдо по поставкам` - ERP-расчет по выбранным поставкам. `Сверка ERP ↔ 1С` - отдельный процесс сравнения двух систем. Бейдж вида `9/9 ОК` в матрице документов не означает, что сальдо ERP равно сальдо 1С.",
        fill=WARN_FILL,
    )
...
def add_workflow(doc):
    ...
    steps = [
        ...
        "Если бухгалтеру нужна проверка учетных систем, он открывает отдельную страницу `Сверка ERP ↔ 1С`.",
        ...
        "Пользователь видит итог: совпало ли сальдо, сколько документов совпало, чего нет в 1С, чего нет в ERP, какие поля расходятся.",
        "Фоновые runner-задачи могут запускать тяжелую историческую сверку по расписанию и сохранять результат для просмотра без пересчета на экране.",
    ]
    for step in steps:
        add_number(doc, step)
...
def add_functional_requirements(doc):
    ...
    add_table(
        ...
        [
            ...
            (
                "FR-06",
                "Страница `Сверка ERP ↔ 1С` запускается отдельно от матрицы сальдо и работает через существующий PHP/SOAP-слой.",
                "Результат страницы - протокол расхождений, а не бухгалтерская сальдовка по поставкам.",
            ),
            (
                "FR-07",
                "Сверка ERP ↔ 1С возвращает статусы `ОК`, `Нет в 1С`, `Нет в ERP`, `Сумма расходится`, `Вопрос по НДС`, `Нет СФ/УПД`, `Сальдо расходится`, `Ошибка источника`.",
                "Пользователь видит причину, ERP-значение, 1С-значение, поле расхождения и ссылку на исходный документ.",
            ),
            (
                "FR-08",
            ...
            (
                "FR-11",
                "Runner может выполнять тяжелые сверки по расписанию или вручную в фоне.",
...
def add_reconciliation_page_requirements(doc):
    add_heading(doc, "3.1 Отдельная страница сверки ERP ↔ 1С", 2)
    add_body(
        doc,
    ...
    add_callout(
        ...
        "Матрица сальдо отвечает на вопрос `сколько клиент должен/переплатил по выбранным поставкам`. Страница сверки ERP ↔ 1С отвечает на вопрос `совпадают ли две учетные системы`. Это разные интерфейсы, разные статусы и разные критерии приемки.",
        fill=LIGHT_FILL,
    )


def add_runner_requirements(doc):
    add_heading(doc, "3.2 Фоновый runner для тяжелых сверок", 2)
    add_body(
        doc,
        "Историческая сверка по всем клиентам и всем договорам не должна выполняться как обычный HTTP-запрос экрана. Для этого нужен runner, который запускается по расписанию или вручную и сохраняет результат.",
    )
    add_table(
        doc,
        ["Требование", "Описание"],
        [
            ...
            ("Актуальность", "у каждого результата показывать дату расчета и источник: live ERP, live 1С, cached 1С, ошибка источника"),
            ("Повторяемость", "один и тот же `run_id` должен хранить использованные входные данные, чтобы итог можно было объяснить позже"),
            ("Экспорт", "XLSX по результату runner формируется из сохраненного результата, а не пересчитывает тяжелые SQL и SOAP заново"),
        ],
        [2300, 7060],
        font_size=7.85,
    )
...
def add_nonfunctional_requirements(doc):
    ...
    add_table(
        ...
        [
            (
                "Производительность",
                "На экране нельзя запускать расчет по всему клиенту без лимита. По умолчанию до 50 поставок на страницу; тяжелые процедуры `get_paidsum/get_realizsum/get_expensessum/get_profit` выполнять только по выбранным `spec_id`. Большие выборки - через пагинацию или фоновую XLSX-задачу.",
            ),
            (
                "Надежность",
            ...
            (
                "Совместимость",
                "Production-реализация остается в текущем PHP ERP + MariaDB + SOAP 1С. Python-скрипты, PostgreSQL, Google Drive/Sheets и тестовые JSON/XLSX не являются частью промышленного контура.",
            ),
            (
                "Наблюдаемость",
...
def add_ui(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Статусы", "Ошибки показываются бейджами с конкретной причиной: сумма, номер, дата, договор, НДС, отсутствие документа. Общий текст `Реквизиты` без поля не использовать."),
    ...
    add_image(doc, IMG_UI, "Рисунок 1. Целевой вид экрана `Сальдо по поставкам` по свежему кейсу 660/1.")
    add_image(doc, IMG_ERRORS, "Рисунок 2. Диагностический режим текущей матрицы: это не заменяет отдельную страницу сверки ERP ↔ 1С.")
...
def add_sources_and_selects(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Размер страницы", "Экран матрицы показывает до 50 поставок за один запрос. Для большего объема нужна пагинация или фоновая XLSX-задача."),
...
""",
    ...
    add_callout(
        ...
        "В репозитории рабочая проверка находится в `tools/server_mariadb_reconciliation_report.py`. Она не является production-кодом, но фиксирует SQL-связи, формулы, структуру данных и правила XLSX, которые нужно реализовать в ERP.",
    )
...
def add_statuses(doc):
    ...
    add_table(
        ...
        [
            ...
            ("Сальдо расходится", "начальное, обороты или конечное сальдо ERP и 1С отличаются", "только для отдельной страницы сверки ERP ↔ 1С"),
    ...
    add_callout(
        doc,
        "Ограничение документного статуса",
        "Статус вида `9/9 ОК` может означать только, что девять сопоставленных документов не дали полевых расхождений. Он не доказывает, что сальдо ERP равно сальдо 1С. Для этого нужен отдельный сальдовый блок сверки.",
        fill=WARN_FILL,
    )
...
def add_control_case(doc):
    ...
    add_callout(
        ...
        "Контрольный кейс 660/1 подтверждает корректность ERP-сальдовки по выбранным поставкам. Он не подтверждает, что сальдо ERP и 1С совпадает по договору за период. Для такого вывода нужен отдельный запуск страницы `Сверка ERP ↔ 1С` с сальдо на начало, оборотами и сальдо на конец.",
        fill=WARN_FILL,
    )
    add_body(
        doc,
...
def add_acceptance(doc):
    ...
    checks = [
        ...
        "Свежий кейс 660/1 воспроизводит live-итоги MariaDB: поступления/get_paidsum 2 331 111,33; реализация/get_realizsum 2 415 525,73; сальдо -84 414,40. Это критерий ERP-сальдовки, не критерий совпадения с 1С.",
        ...
        "На экране действует лимит до 50 поставок на запрос; большие периоды требуют пагинации или фоновой XLSX-задачи.",
        ...
        "Бейдж `1С: X/Y совпало` означает только документное сопоставление и не используется как итоговый статус совпадения сальдо.",
        "Для настоящей сверки ERP ↔ 1С есть отдельная страница с запуском `run_id`, статусом источников, блоком `ERP без 1С`, блоком `1С без ERP`, блоком полевых расхождений и блоком сальдо.",
        ...
        "Runner для тяжелой исторической сверки сохраняет результат и не блокирует интерфейс синхронным пересчетом всей истории.",
