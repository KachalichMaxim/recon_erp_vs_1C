...
- `Дата расходится`;
- `Договор расходится`;
- `Поля документа расходятся`;
- `Недостаточно данных`;
- `PAYMENT_ROUTINE_VS_DOCS_MISMATCH`;
- `EXCLUDED_CONTRACTOR_EXPENSE`.

Бейдж поставки `1С: X/Y совпало` означает результат сравнения строк ERP ↔ 1С: `X` строк совпали из `Y` найденных строк сверки. Бейджи `Долг` и `Переплата` считаются только по формуле `сумма оплат - возмещаемые расходы - невозмещаемые расходы`: отрицательная дельта = долг, положительная = переплата. Общий статус `Реквизиты расходятся` не использовать: backend должен вернуть конкретное поле расхождения.
