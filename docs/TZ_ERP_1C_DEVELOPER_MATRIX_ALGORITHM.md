...
Бейдж поставки `1С: X/Y совпало` означает результат сравнения строк ERP ↔ 1С: `X` строк совпали из `Y` найденных строк сверки. Бейдж `Есть остаток` или `Остаток` считается только по формуле `сумма оплат - возмещаемые расходы - невозмещаемые расходы`. Общий статус `Реквизиты расходятся` не использовать: backend должен вернуть конкретное поле расхождения.
